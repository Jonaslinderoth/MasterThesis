...
std::vector<bool>* BreakingIntervallExperimentNormalData::findDimensionsEx(std::vector<float>* centroid,
...
std::vector<int> BreakingIntervallExperimentNormalData::randIntVec(unsigned int lower,unsigned int upper,unsigned int n){
...
std::vector<std::vector<float>*>* BreakingIntervallExperimentNormalData::pickRandomPointFromData(std::vector<std::vector<float>*>* data, unsigned int size){
...
void BreakingIntervallExperimentNormalData::start(){
	/*
	...
	std::size_t no_of_points = 2000; //per cluster and NEED to be dividable by the clusteres
	...
	for(unsigned long breakIntervall = 1; breakIntervall <= breakIntervallEnd ; breakIntervall+=breakIntervallIncrease){
		...
		if(not prints){
			Experiment::writeLineToFile(std::to_string(no_of_points) + ", " + std::to_string(point_dim) + ", naive break, " + std::to_string(breakIntervall) + ", " + std::to_string(millisReducedReadsBreaking));
			Experiment::testDone("Dim: " + std::to_string(point_dim) + " numPoints: " + std::to_string(no_of_points)+ " breaking interval: " + std::to_string(breakIntervall));
		}
