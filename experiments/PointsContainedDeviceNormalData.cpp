...
void PointsContainedDeviceNormalData::start(){
	...
	versionInfo pointsContainedKernelNaiveBreakVI = {"pointsContainedKernelNaiveBreak",4};
	versionInfo pointsContainedKernelSharedMemoryBreakVI = {"pointsContainedKernelSharedMemoryBreak",5};
	versionInfo whatDataIsInCentroidVI = {"whatDataIsInCentroid",6};
	versionInfo whatDataIsInCentroidFewPointsVI = {"whatDataIsInCentroidFewPoints",7};
	versionInfo whatDataIsInCentroidChunksVI = {"whatDataIsInCentroidChunks",8};
	versionInfo whatDataIsInCentroidLessReadingWrapperVI = {"whatDataIsInCentroidLessReadingWrapper",9};
	versionInfo whatDataIsInCentroidLessReadingAndBreakingWrapperVI = {"whatDataIsInCentroidLessReadingAndBreakingWrapper",10};
	versionInfo whatDataIsInCentroidKernelFewPointsKernelVI = {"whatDataIsInCentroidKernelFewPointsKernel",11};
	...
	versionVec.push_back(whatDataIsInCentroidVI);
	versionVec.push_back(whatDataIsInCentroidFewPointsVI);
	versionVec.push_back(whatDataIsInCentroidChunksVI);
	versionVec.push_back(whatDataIsInCentroidLessReadingWrapperVI);
	versionVec.push_back(whatDataIsInCentroidLessReadingAndBreakingWrapperVI);
	versionVec.push_back(whatDataIsInCentroidKernelFewPointsKernelVI);


	std::vector<unsigned long> maxPoints;
	maxPoints.push_back(1000);
	maxPoints.push_back(10000);
	maxPoints.push_back(25000);
	maxPoints.push_back(50000);

	c = versionVec.size()*maxPoints.size();

	Experiment::addTests(c);

	Experiment::start();

	for(std::vector<unsigned long>::iterator interPointAmount = maxPoints.begin(); interPointAmount != maxPoints.end();++interPointAmount){
		...
		std::size_t no_of_points = *interPointAmount; //NEED to be dividable by the clusteres
		if(prints){
			no_of_points = 20;
		}
		...
		for(std::vector<versionInfo>::iterator iter = versionVec.begin() ; iter != versionVec.end(); ++iter){
			...
			if(iter->version == 0){
				...
				// Call kernel
				whatDataIsInCentroid(ceil((no_of_points)/(float)block_size),
									 block_size,
									 stream,
									 output_d,
									 data_d,
			...
			}else if(iter->version == 7){
				// Call kernel

				whatDataIsInCentroidFewPoints((std::size_t)ceil((no_of_points)/(float)block_size),
			...
			}else if(iter->version == 8){
				// Call kernel
				whatDataIsInCentroidChunks(ceil((no_of_points)/(float)block_size),
										   block_size,
										   stream,
										   output_d,
										   data_d,
			...
			}else if(iter->version == 9){
				// Call kernel
				whatDataIsInCentroidLessReadingWrapper(ceil((no_of_points)/(float)block_size),
													   block_size,
													   stream,
													   output_d,
													   data_d,
			...
			}else if(iter->version == 10){
				// Call kernel
				whatDataIsInCentroidLessReadingAndBreakingWrapper(ceil((no_of_points)/(float)block_size),
															      block_size,
															      stream,
															      output_d,
															      data_d,
			...
			}else if(iter->version == 11){
				// Call kernel
				whatDataIsInCentroidKernelFewPointsKernel(ceil((no_of_points)/(float)block_size),
														  block_size,
														  stream,
														  output_d,
														  data_d,
