...
TEST(testMineClus, testFind2ClusterSimple1){
	...
	EXPECT_EQ(res.at(0).first->at(1)->at(0), 2);
	EXPECT_EQ(res.at(0).first->at(2)->at(0), 3);
	EXPECT_EQ(res.at(0).first->at(3)->at(0), 4);
	...
	EXPECT_EQ(res.at(1).first->at(0)->at(0), 50);
	EXPECT_EQ(res.at(1).first->at(1)->at(0), 51);
	EXPECT_EQ(res.at(1).first->at(2)->at(0), 52);
...
TEST(testMineClus, testFind2ClusterSimple1Concurent){
	auto data = new std::vector<std::vector<float>*>;
	{
		auto point = new std::vector<float>{1,10000};
		data->push_back(point);
	...
	EXPECT_EQ(res.at(2).first->size(), 1);
	EXPECT_EQ(res.at(2).first->at(0)->at(0), 10000);
	EXPECT_TRUE(res.at(2).second->at(0));
	EXPECT_TRUE(res.at(2).second->at(1));
}
...
TEST(testMineClus, _SLOW_testFind5ClustersConcurent){
	unsigned int dim = 20;
	auto data =  new std::vector<std::vector<float>*>();
