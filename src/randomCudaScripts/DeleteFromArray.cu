...
__global__ void gpuDeleteFromArray(unsigned int* const d_outData, const unsigned int* delete_array, const float* data,unsigned int* indexes , const size_t numElements){
...
void deleteFromArray(const float* d_outData, const bool* delete_array, const float* data,unsigned int* indexes, const unsigned long numElements){
	...
	checkCudaErrors(cudaMalloc(&d_out_blelloch, sizeof(unsigned int) * numElements));

	/*
	sum_scan_blelloch(d_out_blelloch,delete_array,numElements);
	*/
