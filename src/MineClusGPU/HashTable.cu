...
void findDublicatesHashTableTester(unsigned int dimGrid, unsigned int dimBlock,
	...
	unsigned int sizeOfHashTable = pow(2,ceilf(log2(numberOfCandidates)));
	assert(sizeOfHashTable >= numberOfCandidates);
	auto hashTable = create_hashtable(sizeOfHashTable);
	insertHashTable<<<dimGrid, dimBlock>>>(hashTable, candidates, sizeOfHashTable, numberOfCandidates, dim, alreadyDeleted_d, output_d);
